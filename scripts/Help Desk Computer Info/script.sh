...
if [ -d /Applications/Utilities/Help\ Desk\ Computer\ Information.app ]
then
	open /Applications/Utilities/Help\ Desk\ Computer\ Information.app
else
...
# Display free space
FreeSpace=$( /usr/sbin/diskutil info / | /usr/bin/grep  -E 'Free Space|Available Space' | /usr/bin/awk -F ":\s*" '{ print $2 }' | awk -F "(" '{ print $1 }' | xargs )
FreePercentage=$( /usr/sbin/diskutil info / | /usr/bin/grep -E 'Free Space|Available Space' | /usr/bin/awk -F "(\\\(|\\\))" '{ print $6 }' )
...
# Display the information, Optionally havd a second button that can run some additional script
# runCommand="button returned of (display dialog \"$displayInfo\" with title \"Computer Information Script\" with icon file posix file \"/System/Library/CoreServices/Finder.app/Contents/Resources/Finder.icns\" buttons {\"Extra Option\", \"OK\"} default button {\"OK\"})"
runCommand="button returned of (display dialog \"$displayInfo\" with title \"Computer Information Script\" with icon file posix file \"/System/Library/CoreServices/Finder.app/Contents/Resources/Finder.icns\" buttons {\"OK\"} default button {\"OK\"})"
